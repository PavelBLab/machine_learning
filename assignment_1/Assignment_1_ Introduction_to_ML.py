...
'=================== Question 0 (Example) ==================='
print('='.center(80, '='))
# TODO: You should write your whole answer within the function provided.
def answer_zero():
    # This function returns the number of features of the breast cancer dataset, which is an integer.
    print(pd.DataFrame(cancer['data']).shape, '\n',
          pd.DataFrame(cancer['data']).head(), '\n',
          '='.center(40, '='), '\n',
          pd.DataFrame(cancer['feature_names']).shape,
          pd.DataFrame(cancer['feature_names']).head(),
          '='.center(40, '='), '\n',
          pd.DataFrame(cancer['target']).shape,
...
def answer_one():
    ...
    df['target'] = pd.Series(cancer['target'], index = df.index)
    return df
answer_one()
# print('Shape ==>', answer_one().shape, '\n',
#       answer_one().columns, '\n',
#       answer_one().head())
...
def answer_two():
    cancerdf = answer_one()
    # print('Shape ==>', cancerdf['target'].shape)
    malignant = np.count_nonzero(cancerdf['target'] == 0)
    benign = np.count_nonzero(cancerdf['target'])
    # print(malignant + benign)
    # print(cancer['target_names'])
    target = pd.Series([malignant, benign], index=cancer['target_names'])
...
This function should return a numpy array either array([ 0.]) => 'malignant' or array([ 1.]) => 'benign'
'''
...
def answer_six():
    ...
    means = cancerdf.mean()[:-1].values.reshape(1, -1) # https://stackoverflow.com/questions/18691084/what-does-1-mean-in-numpy-reshape
    # print(means.shape)
